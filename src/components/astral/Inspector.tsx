  ...
  return (
    <div className="astral-insp">
      <div className="ih">
        ...
        <div className="x" onClick={() => S.setSel(null)}>
          ✕ deselect
        </div>
      </div>
      ...
      {["Wall", "Building", "RoofSystem", "Slab", "Opening"].includes(n.type) && (
        <button
          ...
          style={{ fontSize: 12, padding: "8px 14px", marginBottom: 6 }}
          onClick={() => {
            S.setDetailFor(sel);
            S.set("view", "detail");
          }}
        ...
        <>
          ...
          {S.units > 1 && (
            <div className="astral-muted" style={{ fontSize: 11, marginTop: -4 }}>
              Party walls split the footprint into {S.units} equal bays along the length. Each unit
              ≈ {Math.round(S.L / S.units)}mm.
            </div>
          )}
        </>
      )}
      {n.type === "Slab" && (
        <>
          <OptRow
            ...
            current={S.found}
            onPick={(v) => S.set("found", v)}
          />
          {S.found === "Timber piles" && (
            <DimInput
              label="Subfloor clearance"
              getMM={() => S.subfloor}
              setMM={(v) => S.set("subfloor", v)}
              minMM={300}
              maxMM={1500}
            />
          )}
      ...
      {n.type === "RoofSystem" && (
        <>
          ...
          <OptRow
            ...
            current={S.cover}
            onPick={(v) => S.set("cover", v)}
          />
          <OptRow
            label="Structure"
            values={["Prefab trusses", "Cut roof"] as const}
            current={S.struct}
            onPick={(v) => S.set("struct", v)}
          />
        </>
      )}
      {n.type === "Opening" &&
        (() => {
          ...
          return (
            <>
              <OptRow
                label="Type"
                values={["Garage", "Door", "Window"] as const}
                current={o.kind}
                onPick={(v) =>
                  S.updateOpening(o.id, {
                    kind: v,
              ...
              <OptRow
                label="Wall"
                values={(["S", "N", "W", "E"] as const).map((w) => WALLNAME[w])}
              ...
              <DimInput
                label="Width"
                getMM={() => o.width}
                setMM={(v) => S.updateOpening(o.id, { width: v })}
                minMM={400}
                maxMM={7000}
              />
              <DimInput
                label="Set-out to centre"
                getMM={() => o.off}
                setMM={(v) => S.updateOpening(o.id, { off: v })}
                minMM={0}
                maxMM={20000}
              />
              <DimInput
                ...
                minMM={600}
                maxMM={Math.max(600, S.studH - 90)}
              />
              {o.kind === "Window" && (
                <DimInput
                  label="Sill (bottom AFFL)"
                  getMM={() => sill}
                  setMM={(v) => S.updateOpening(o.id, { sill: v })}
                  minMM={0}
                  maxMM={Math.max(0, head - 200)}
                />
              )}
              {(headOver || sillOver || overlaps || overflow) && (
                <div
                  className="astral-muted"
                  style={{ fontSize: 11, color: "#b4472d", marginTop: 4 }}
                >
                  {headOver && (
                    <>
                      ⚠ Head {head} exceeds {hMax} (studH − lintel − top plate). Lower head or use
                      engineered design.
                      <br />
                    </>
                  )}
                  {sillOver && (
                    <>
                      ⚠ Sill must sit at least 200mm below the head.
                      <br />
                    </>
                  )}
                  {overlaps && (
                    <>
                      ⚠ Overlaps another opening on this wall — move or resize.
                      <br />
                    </>
                  )}
                  {overflow && (
                    <>⚠ Extends past the wall ends — reduce width or move the set-out.</>
                  )}
                </div>
              )}
            </>
        ...
        (() => {
          ...
          return (
            <>
              <OptRow
                label="Direction"
                values={["Across width", "Along length"] as const}
                current={p.dir}
                onPick={(v) =>
                  S.updatePartition(p.id, {
                    dir: v,
                    start: 0,
                    len: v === "Across width" ? S.W : S.L,
                  })
                }
              />
              <DimInput
                ...
                getMM={() => p.off}
                setMM={(v) => S.updatePartition(p.id, { off: v })}
                minMM={0}
                maxMM={posMax}
              />
              <DimInput
                label="Start"
                getMM={() => p.start}
                setMM={(v) => S.updatePartition(p.id, { start: v })}
                minMM={0}
                maxMM={runMax}
              />
              <DimInput
                label="Length"
                getMM={() => p.len}
                setMM={(v) => S.updatePartition(p.id, { len: v })}
                minMM={200}
                maxMM={runMax}
              />
            </>
          ...
          return (
            <>
              ...
              <p className="astral-muted" style={{ fontSize: 11, marginTop: 4 }}>
                Affects line/hatch on the architectural plan (§7.1 legend). Length and structural
                derivation still come from the Building.
              </p>
            </>
          );
        })()}
