...
export function AxonView() {
  const S = useAstral(useShallow((s) => ({
    L: s.L, W: s.W, studH: s.studH, storeys: s.storeys, floorDepth: s.floorDepth,
    found: s.found, subfloor: s.subfloor, roof: s.roof, pitch: s.pitch,
    clad: s.clad, cladCol: s.cladCol, roofCol: s.roofCol, openings: s.openings,
    eave: s.eave, units: s.units,
  })));
  ...
  const view = useMemo(() => {
    ...
    const corners: [number, number, number][] = [
      ...
      [0, 0, totalH + apexH], [L, 0, totalH + apexH],
      [L, W, totalH + apexH], [0, W, totalH + apexH],
    ];
  ...
  const gp = [
    P(-gPad, -gPad, 0), P(L + gPad, -gPad, 0),
    P(L + gPad, W + gPad, 0), P(-gPad, W + gPad, 0),
  ];
  ...
  if (piles) {
    ...
    for (let x = 0; x <= L + 1; x += stepX) {
      for (let y = 0; y <= W + 1; y += stepY) {
        const a = P(x - 90, y - 90, 0), b = P(x + 90, y - 90, 0);
        const c = P(x + 90, y + 90, 0), d = P(x - 90, y + 90, 0);
        const at = P(x - 90, y - 90, groundZ), bt = P(x + 90, y - 90, groundZ);
        const ct = P(x + 90, y + 90, groundZ), dt = P(x - 90, y + 90, groundZ);
        foundation.push(
          <g key={`pile${x}-${y}`} pointerEvents="none">
            <polygon points={`${at[0]},${at[1]} ${bt[0]},${bt[1]} ${ct[0]},${ct[1]} ${dt[0]},${dt[1]}`}
              fill="#cdbf9f" stroke={INK} strokeWidth={0.4} />
            <polygon points={`${a[0]},${a[1]} ${b[0]},${b[1]} ${bt[0]},${bt[1]} ${at[0]},${at[1]}`}
              fill={shade("#cdbf9f", 0.78)} stroke={INK} strokeWidth={0.3} />
            <polygon points={`${b[0]},${b[1]} ${c[0]},${c[1]} ${ct[0]},${ct[1]} ${bt[0]},${bt[1]}`}
              fill={shade("#cdbf9f", 0.88)} stroke={INK} strokeWidth={0.3} />
          </g>,
        );
      }
    }
    ...
    // slab block
    const z0 = -150, z1 = floorH; // 150mm embedment hint
    const top = [P(0, 0, z1), P(L, 0, z1), P(L, W, z1), P(0, W, z1)];
    const frontA = P(0, 0, z0), frontB = P(L, 0, z0);
    const rightA = P(L, W, z0);
    foundation.push(
      <g key="slab" pointerEvents="none">
        <polygon points={`${frontA[0]},${frontA[1]} ${frontB[0]},${frontB[1]} ${top[1][0]},${top[1][1]} ${top[0][0]},${top[0][1]}`}
          fill={shade("#efeadd", 0.82)} stroke={INK} strokeWidth={0.6} />
        <polygon points={`${frontB[0]},${frontB[1]} ${rightA[0]},${rightA[1]} ${top[2][0]},${top[2][1]} ${top[1][0]},${top[1][1]}`}
          fill={shade("#efeadd", 0.9)} stroke={INK} strokeWidth={0.6} />
        <polygon points={`${top[0][0]},${top[0][1]} ${top[1][0]},${top[1][1]} ${top[2][0]},${top[2][1]} ${top[3][0]},${top[3][1]}`}
          fill="#efeadd" stroke={INK} strokeWidth={0.6} />
      </g>,
    );
  }
  ...
  for (let s = 0; s < S.storeys; s++) {
    ...
    const corners = {
      A: P(0, 0, z0), B: P(L, 0, z0), C: P(L, W, z0), D: P(0, W, z0),
      At: P(0, 0, z1), Bt: P(L, 0, z1), Ct: P(L, W, z1), Dt: P(0, W, z1),
    };
    // front (y=0) — visible
    storeyFaces.push(
      <polygon key={`fr${s}`} data-node-id="bldg" data-node-type="Building"
        points={`${corners.A[0]},${corners.A[1]} ${corners.B[0]},${corners.B[1]} ${corners.Bt[0]},${corners.Bt[1]} ${corners.At[0]},${corners.At[1]}`}
        fill={cladLeft} stroke={INK} strokeWidth={0.7} />,
    );
    // right (x=L) — visible
    storeyFaces.push(
      <polygon key={`rt${s}`}
        points={`${corners.B[0]},${corners.B[1]} ${corners.C[0]},${corners.C[1]} ${corners.Ct[0]},${corners.Ct[1]} ${corners.Bt[0]},${corners.Bt[1]}`}
        fill={cladRight} stroke={INK} strokeWidth={0.7} />,
    );
    ...
    if (s === S.storeys - 1 && S.roof === "Flat") {
      storeyFaces.push(
        <polygon key={`tp${s}`}
          ...
          fill={roofC} stroke={INK} strokeWidth={0.7} />,
      );
    }
    ...
    sourceOps.forEach((o, idx) => {
      ...
      const headH = o.head ?? (o.kind === "Garage" ? Math.min(storeyH - 200, 2400) : o.kind === "Door" ? 1980 : 2000);
      ...
      const off = o.off;
      if (o.wall === "S") {
        const p1 = P(off - ow / 2, 0, oz0), p2 = P(off + ow / 2, 0, oz0);
        const p3 = P(off + ow / 2, 0, oz1), p4 = P(off - ow / 2, 0, oz1);
        storeyFaces.push(
          <polygon key={`op-s${s}-${idx}`}
            points={`${p1[0]},${p1[1]} ${p2[0]},${p2[1]} ${p3[0]},${p3[1]} ${p4[0]},${p4[1]}`}
            fill={o.kind === "Garage" ? "#22303a" : "#3f5560"} stroke={INK} strokeWidth={0.4} opacity={0.95}
            data-node-id={`opening-${o.id}`} data-node-type="Opening" />,
        );
      } else if (o.wall === "E") {
        const p1 = P(L, off - ow / 2, oz0), p2 = P(L, off + ow / 2, oz0);
        const p3 = P(L, off + ow / 2, oz1), p4 = P(L, off - ow / 2, oz1);
        storeyFaces.push(
          <polygon key={`op-e${s}-${idx}`}
            points={`${p1[0]},${p1[1]} ${p2[0]},${p2[1]} ${p3[0]},${p3[1]} ${p4[0]},${p4[1]}`}
            fill={o.kind === "Garage" ? "#22303a" : "#3f5560"} stroke={INK} strokeWidth={0.4} opacity={0.95}
            data-node-id={`opening-${o.id}`} data-node-type="Opening" />,
        );
    ...
    if (s < S.storeys - 1) {
      const fz0 = z1, fz1 = z1 + interH;
      const A = P(0, 0, fz0), B = P(L, 0, fz0), Bt = P(L, 0, fz1), At = P(0, 0, fz1);
      const C = P(L, W, fz0), Ct = P(L, W, fz1);
      storeyFaces.push(
        <g key={`fl${s}`} pointerEvents="none">
          <polygon points={`${A[0]},${A[1]} ${B[0]},${B[1]} ${Bt[0]},${Bt[1]} ${At[0]},${At[1]}`}
            fill={shade(cladLeft, 0.72)} stroke={INK} strokeWidth={0.5} />
          <polygon points={`${B[0]},${B[1]} ${C[0]},${C[1]} ${Ct[0]},${Ct[1]} ${Bt[0]},${Bt[1]}`}
            fill={shade(cladRight, 0.78)} stroke={INK} strokeWidth={0.5} />
        </g>,
      );
    }
  }
  ...
  if (S.roof !== "Flat") {
    ...
    if (S.roof === "Mono") {
      const A = P(0, 0, topZ), B = P(L, 0, topZ), C = P(L, W, topZ), D = P(0, W, topZ);
      const At = P(0, 0, apex), Bt = P(L, 0, apex);
      roofShape.push(
        <g key="roof" pointerEvents="none">
          <polygon points={`${A[0]},${A[1]} ${B[0]},${B[1]} ${Bt[0]},${Bt[1]} ${At[0]},${At[1]}`}
            fill={shade(roofC, 1.05)} stroke={INK} strokeWidth={0.7} />
          <polygon points={`${At[0]},${At[1]} ${Bt[0]},${Bt[1]} ${C[0]},${C[1]} ${D[0]},${D[1]}`}
            fill={roofC} stroke={INK} strokeWidth={0.7} />
        </g>,
      );
    } else {
      // gable / hip — ridge along long axis
      const ridge0 = P(0, W / 2, apex), ridge1 = P(L, W / 2, apex);
      const A = P(0, 0, topZ), B = P(L, 0, topZ), C = P(L, W, topZ), D = P(0, W, topZ);
      roofShape.push(
        <g key="roof" pointerEvents="none">
          {/* front slope */}
          <polygon points={`${A[0]},${A[1]} ${B[0]},${B[1]} ${ridge1[0]},${ridge1[1]} ${ridge0[0]},${ridge0[1]}`}
            fill={shade(roofC, 1.08)} stroke={INK} strokeWidth={0.7} />
          {/* right gable triangle (visible) */}
          <polygon points={`${B[0]},${B[1]} ${C[0]},${C[1]} ${ridge1[0]},${ridge1[1]}`}
            fill={shade(cladRight, 1.02)} stroke={INK} strokeWidth={0.7} />
          {/* back slope (partial visibility) */}
          <polygon points={`${ridge0[0]},${ridge0[1]} ${ridge1[0]},${ridge1[1]} ${C[0]},${C[1]} ${D[0]},${D[1]}`}
            fill={roofC} stroke={INK} strokeWidth={0.7} opacity={0.92} />
        </g>,
      );
    }
  }
    ...
    <ClickLayer>
      <svg viewBox="-14 -14 628 540" preserveAspectRatio="xMidYMid meet">
        ...
        <polygon
          ...
          fill="#f1ecde" stroke="#c8bfa3" strokeWidth={0.4} pointerEvents="none"
        />
        {/* ground grid lines (one per metre) */}
        {Array.from({ length: 9 }).map((_, i) => {
          const t = -gPad + (i + 1) * (L + 2 * gPad) / 10;
          const a = P(t, -gPad, 0), b = P(t, W + gPad, 0);
          return <line key={`gg${i}`} x1={a[0]} y1={a[1]} x2={b[0]} y2={b[1]}
            stroke="#d6cdb2" strokeWidth={0.3} pointerEvents="none" />;
        })}
        ...
        {S.units > 1 && S.roof !== "Flat" && Array.from({ length: S.units - 1 }).map((_, i) => {
          const x = (L / S.units) * (i + 1);
          const a = P(x, W / 2, groundZ + floorH + storeyH * S.storeys + interH * (S.storeys - 1) + apexH);
          const b = P(x, W / 2, groundZ);
          return <line key={`pw${i}`} x1={a[0]} y1={a[1]} x2={b[0]} y2={b[1]}
            stroke={INK} strokeWidth={0.5} strokeDasharray="3 3" pointerEvents="none" />;
        })}
        ...
        <g transform={`translate(${nA[0]} ${nA[1]})`} pointerEvents="none">
          ...
          <text y={-13} fontFamily={MONO} fontSize={7} fill={INK} textAnchor="middle">N</text>
        </g>

        {/* legend */}
        <text x={VW / 2} y={26} fill="#3c4a47" fontSize={10} fontFamily={MONO} textAnchor="middle">
          AXONOMETRIC · {S.storeys}-STOREY · {piles ? `TIMBER PILE FLOOR (${S.subfloor}mm subfloor)` : "CONCRETE SLAB"}
        </text>
        <text x={VW / 2} y={VH - 8} fill="#7a6f57" fontSize={8} fontFamily={MONO} textAnchor="middle">
          indicative massing · same model as plan / elevation / section · not to scale
        </text>

        <SheetFrame />
      </svg>
      <ScaleChip label={`axonometric · ${S.storeys}-storey · ${piles ? "piled" : "slab"} · roof ${S.roof}`} />
    </ClickLayer>
  );
}
