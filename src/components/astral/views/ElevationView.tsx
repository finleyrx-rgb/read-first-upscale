...
export function ElevationView() {
  ...
  if (S.roof === "Flat") {
    ...
    pts = [
      [x0 - eo, eave + (eo * apexP) / (w / 2 || 1)],
      [x0 + w / 2, eave - apexP],
      [x0 + w + eo, eave + (eo * apexP) / (w / 2 || 1)],
    ];
  } else {
  ...
  const lName =
    S.layer === "framing" ? "framing" : S.layer === "foundation" ? "concrete" : "envelope";
  ...
  return (
    <ClickLayer>
      <svg viewBox="-14 -14 628 540" preserveAspectRatio="xMidYMid meet">
        ...
        {S.layer === "foundation" && (
          <FoundationLayer x0={x0} w={w} eave={eave} eh={data.eh} base={base} />
        )}
        {S.layer === "framing" && (
          <FramingLayer
            S={S}
        ...
        <text
          x={x0 + w / 2}
          y={base + (S.layer === "foundation" ? 36 : 18)}
          fill="#3c4a47"
          fontSize={11}
          fontFamily="IBM Plex Mono"
          textAnchor="middle"
        >
          {fName} · {S.roof}
          {S.roof === "Flat" && S.parapet ? " + parapet" : ""} {S.pitch}° ·{" "}
          {S.layer === "framing"
            ? "framing"
            : S.layer === "foundation"
              ? "concrete"
...
function FoundationLayer({
  ...
  return (
    <g>
      ...
      <text x={x0 + w + 26} y={base + 3} fill="#3c4a47" fontSize={9} fontFamily="IBM Plex Mono">
        FFL 0
      </text>
      {/* slab body */}
      <rect
      ...
      <text x={x0 + w + 26} y={fgl + 3} fill="#7a6f57" fontSize={8} fontFamily="IBM Plex Mono">
        FGL
      </text>
      {hatch}
      {anchors}
...
type ElevProps = {
  S: {
    ...
    cladCol: string;
    roofCol: string;
    openings: ReturnType<typeof useAstral.getState>["openings"];
    face: ReturnType<typeof useAstral.getState>["face"];
    layer: ReturnType<typeof useAstral.getState>["layer"];
  };
  pts: [number, number][];
  x0: number;
  w: number;
  eave: number;
  base: number;
  ows: (o: ElevProps["S"]["openings"][number]) => {
    owid: number;
    cxp: number;
    oh: number;
    sill: number;
        ...
        return (
          <g key={`b${i}`}>
            <rect
              ...
              y={eave + 5}
              width={bx2 - bx1}
              height={base - 3 - (eave + 5)}
              fill="rgba(63,125,84,0.07)"
              stroke="#3f7d54"
              strokeWidth={0.8}
            />
            <line
        ...
        return (
          <g key={o.id}>
            <rect
              ...
              opacity={0.55}
              stroke={col}
              strokeWidth={o.kind === "Garage" ? 2.4 : 1.6}
...
function CladTexture({
  ...
  if (kind === "Weatherboard") {
    for (let yy = y + 6; yy < y + h; yy += 6)
      lines.push(
        <line key={yy} x1={x} y1={yy} x2={x + w} y2={yy} stroke="#00000022" strokeWidth={0.6} />,
      );
  } else if (kind === "Ply + batten") {
    for (let xx = x + 10; xx < x + w; xx += 20)
      lines.push(
        <line key={xx} x1={xx} y1={y} x2={xx} y2={y + h} stroke="#00000022" strokeWidth={0.8} />,
      );
  } else if (kind === "Fibre-cement") {
    for (let xx = x + 24; xx < x + w; xx += 24)
      lines.push(
        <line key={xx} x1={xx} y1={y} x2={xx} y2={y + h} stroke="#00000018" strokeWidth={0.5} />,
      );
  }
  return (
    <g>
...
function RoofTexture({
  ...
  const lines: React.ReactElement[] = [];
  if (step)
    for (let xx = minx + step; xx < maxx; xx += step)
      lines.push(
        <line key={xx} x1={xx} y1={miny} x2={xx} y2={maxy} stroke="#ffffff22" strokeWidth={0.6} />,
      );
  return (
    <g>
      <polygon
