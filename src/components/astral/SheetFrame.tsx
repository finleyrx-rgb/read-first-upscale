...
type TProps = {
  ...
  s?: number; c?: string; f?: string; a?: "start" | "middle" | "end"; w?: number;
};
function T({ x, y, t, s = 9, c = INK, f = "IBM Plex Mono", a = "start", w }: TProps) {
  return (
    <text x={x} y={y} fill={c} fontSize={s} fontFamily={f} textAnchor={a}
      fontWeight={w}>{t}</text>
  );
}
...
export function SheetFrame() {
  ...
  return (
    <g pointerEvents="none" aria-hidden>
      {/* outer trim edge */}
      <rect x={G.paper.x} y={G.paper.y} width={G.paper.w} height={G.paper.h}
        fill="none" stroke="#d8cfbc" strokeWidth={0.6} />
      {/* doubled inner field rule (top + sides only, stops at title block) */}
      <path d={`M -2 ${G.tbTop} L -2 -2 L 602 -2 L 602 ${G.tbTop}`}
        fill="none" stroke={LINE} strokeWidth={0.5} />
      {/* corner registration ticks */}
      {([
        [-6, -6, 1, 1], [606, -6, -1, 1], [-6, 516, 1, -1], [606, 516, -1, -1],
      ...
      <rect x={G.frame.x} y={G.tbTop} width={G.frame.w} height={G.bottom - G.tbTop}
        fill="#fbf9f3" />
      ...
      <T x={G.metaDiv[0] + 8} y={455} t={`${meta.series} SERIES · ${meta.number}`} s={7} c="#7a6f57" />
      <T x={G.metaDiv[1] + 8} y={455} t={`REV ${meta.rev} · ISSUED ${meta.date}`} s={7} c="#9a8f78" />
      {/* col 0 — identity */}
      ...
      <T x={C[1] + 8} y={486} t={`${meta.project.toUpperCase()} — ${meta.location.toUpperCase()}`} s={10} />
      <T x={C[1] + 8} y={499} t={meta.dims} s={7.5} c="#3c4a47" />
      <T x={C[1] + 8} y={511} t={`WIND ZONE — ${meta.wind.toUpperCase()}`} s={7.5}
        c={hot ? "#bf6b2c" : "#3c4a47"} />
      {/* col 2 — drawing */}
