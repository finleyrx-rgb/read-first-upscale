        ...
        {svc.wet && svc.hwc && svc.sink && svc.basin && svc.wc && (
          <g pointerEvents="none">
            ...
            <polyline
              points={[
                svc.hwc,
                { x: svc.sink.x - 6, y: svc.sink.y },
                { x: svc.basin.x - 6, y: svc.basin.y },
              ]
                .map((p) => `${p.x},${p.y}`)
                .join(" ")}
              fill="none"
            ...
            {svc.gt && (
              <>
                <polyline
                  points={[svc.sink, { x: svc.sink.x + 8, y: svc.sink.y }, svc.gt]
                    .map((p) => `${p.x},${p.y}`)
                    .join(" ")}
                  fill="none"
                  stroke={WASTE}
                ...
                />
                <polyline
                  points={[svc.basin, { x: svc.basin.x + 8, y: svc.basin.y }, svc.gt]
                    .map((p) => `${p.x},${p.y}`)
                    .join(" ")}
                  fill="none"
                  stroke={WASTE}
                ...
                <line
                  ...
                  strokeWidth={1.6}
                />
                <text x={x0 + w + 42} y={svc.wc.y + 3} fill={DRAIN} fontSize={8} fontFamily={MONO}>
                  DN100 SOIL
                </text>
      ...
      {items.map(([lab, col, kind, glyph], i) => {
        ...
        return (
          <g key={lab}>
            ...
            <text x={bx + 22} y={yy} fill={INK} fontSize={8.5} fontFamily={MONO}>
              {lab}
            </text>
          </g>
        );
