    ...
    <>
      <div className="astral-q">
        ...
        <div className="astral-opts">
          {TEMPLATES.map((tp, i) => (
            <div key={tp.n} className="astral-opt" onClick={() => loadTemplate(i)}>
              {tp.n}
            </div>
          ))}
        </div>
      ...
      <OptRow
        label="Building type"
        values={["Garage", "Sleepout", "Cabin", "Studio", "Workshop"] as const}
        current={s.type}
        onPick={applyType}
      />
      <OptRow
        label="Wind / exposure"
        values={["Medium", "High", "Very High", "Extra High", "Cyclonic (SED)"] as const}
        current={s.wind}
        onPick={(v) => set("wind", v)}
      />
      <OptRow
        label="Foundation"
        values={["Concrete slab", "Timber piles"] as const}
        current={s.found}
        onPick={(v) => set("found", v)}
      />
    </>
  );
}
...
function StepFootprint() {
  ...
  return (
    <>
      ...
      />
      <OptRow
        label="Stud spacing (c/c)"
        values={[400, 600] as const}
        current={spacing}
      ...
      {storeys > 1 && (
        <DimInput
          label="Inter-floor depth"
          getMM={() => floorDepth}
          setMM={(v) => set("floorDepth", v)}
          minMM={200}
          maxMM={500}
        />
      )}
      {found === "Timber piles" && (
        <DimInput
          label="Subfloor clearance"
          getMM={() => subfloor}
          setMM={(v) => set("subfloor", v)}
          minMM={300}
          maxMM={1500}
        />
      )}
...
function StepRoof() {
  ...
  return (
    <>
      <OptRow
        label="Roof form"
        values={["Gable", "Hip", "Mono", "Flat"] as const}
        current={s.roof}
        onPick={(v) => set("roof", v)}
      />
      {s.roof !== "Flat" && (
      ...
      <OptRow
        label="Covering"
        values={["Colorsteel corrugate", "Colorsteel tray", "Membrane", "Tile"] as const}
        current={s.cover}
        onPick={(v) => set("cover", v)}
      />
      <OptRow
        label="Structure"
        values={["Prefab trusses", "Cut roof"] as const}
        current={s.struct}
        onPick={(v) => set("struct", v)}
      />
      <Swatches
        label="Roof colour"
        list={ROOFC}
        current={s.roofCol}
        onPick={(n) => set("roofCol", n)}
      />
      <DimInput
        label="Eave overhang"
        getMM={() => s.eave}
...
function StepOpenings() {
  ...
  return (
    <>
          ...
          <div key={o.id} className="astral-openrow">
            <div className="top">
              <div className="kind">
                {(["Garage", "Door", "Window"] as const).map((k) => (
                  <div
                    key={k}
                    className={`astral-mini${o.kind === k ? " sel" : ""}`}
                    onClick={() =>
                      upd(o.id, {
                        kind: k,
                        width: k === "Garage" ? 5000 : k === "Window" ? 1200 : 810,
                      })
                    }
                  >
                    {k}
              ...
              <button className="astral-rm" onClick={() => rm(o.id)}>
                remove
              </button>
            </div>
            ...
            <div className="astral-opts">
              {(["S", "N", "W", "E"] as const).map((wl) => (
                <div
                  key={wl}
                  className={`astral-opt${o.wall === wl ? " sel" : ""}`}
            ...
            <DimInput
              getMM={() => o.off}
              setMM={(v) => upd(o.id, { off: Math.max(0, Math.min(wlen, v)) })}
              minMM={0}
              maxMM={20000}
            />
            ...
            <DimInput
              getMM={() => o.width}
              setMM={(v) => upd(o.id, { width: Math.max(400, Math.min(7000, v)) })}
              minMM={400}
              maxMM={7000}
            />
          </div>
      ...
      <button className="astral-addbtn" onClick={add}>
        + Add opening
      </button>
      <OptRow
        label="Joinery timber"
        values={["Fiji mahogany", "Cedar", "Aluminium", "Pine"] as const}
        current={joinery}
        onPick={(v) => set("joinery", v)}
      />
    </>
  );
}
      ...
      {parts.map((p) => {
        ...
        return (
          <div key={p.id} className="astral-openrow">
            <div className="top">
              <div className="kind">
                {(["Across width", "Along length"] as const).map((k) => (
                  <div
                    key={k}
                    className={`astral-mini${p.dir === k ? " sel" : ""}`}
                    onClick={() =>
                      upd(p.id, { dir: k, start: 0, len: k === "Across width" ? W : L })
                    }
                  >
                    {k}
                  </div>
              ...
              <button className="astral-rm" onClick={() => rm(p.id)}>
                remove
              </button>
            </div>
            ...
            <DimInput
              getMM={() => p.off}
              setMM={(v) => upd(p.id, { off: Math.max(0, Math.min(posMax, v)) })}
              minMM={0}
              maxMM={posMax}
            />
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              ...
              <DimInput
                getMM={() => p.len}
                setMM={(v) => upd(p.id, { len: Math.max(200, Math.min(runMax - p.start, v)) })}
                minMM={200}
                maxMM={runMax}
              />
            </div>
            ...
            <div className="astral-opts">
              {WALLTYPE_KEYS.map((k) => (
                <div
                  key={k}
                  className={`astral-opt${p.type === k ? " sel" : ""}`}
            ...
            <div className="astral-opts">
              {(["Yes", "No"] as const).map((v) => (
                <div
                  key={v}
                  className={`astral-opt${(p.door ? "Yes" : "No") === v ? " sel" : ""}`}
                  style={{ padding: "6px 12px", fontSize: 12 }}
            ...
            {p.door && (
              <>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  <DimInput
                    getMM={() => p.doorOff}
                    setMM={(v) => upd(p.id, { doorOff: Math.max(0, Math.min(runMax, v)) })}
                    minMM={0}
                    maxMM={runMax}
                  />
                  <DimInput
                    getMM={() => p.doorW}
                    setMM={(v) => upd(p.id, { doorW: Math.max(600, Math.min(1200, v)) })}
                    minMM={600}
                    maxMM={1200}
                  />
                </div>
  ...
  return (
    <>
      <OptRow
        ...
        values={["Plaster system", "Weatherboard", "Ply + batten", "Fibre-cement"] as const}
        current={s.clad}
        onPick={(v) => set("clad", v)}
      />
      <Swatches
        ...
        list={CLAD}
        current={s.cladCol}
        onPick={(n) => set("cladCol", n)}
      />
      <OptRow
        label="Interior lining"
        values={["Plywood", "Plasterboard", "None"] as const}
        current={s.lining}
        onPick={(v) => set("lining", v)}
      />
      <OptRow
        label="Floor finish"
        values={["Sealed / clear", "Epoxy", "Polished", "Bare"] as const}
        current={s.floor}
        onPick={(v) => set("floor", v)}
      />
      <OptRow
        label="Insulation"
        values={["None — open", "Ceiling only", "Walls + ceiling"] as const}
        current={s.insul}
        onPick={(v) => set("insul", v)}
      />
    </>
  );
}
  ...
  return (
    <div className="astral-res">
      <div className="rh">
        <span>{label}</span>
        {r.sed ? (
          <span className="astral-tag sed">SED / by others</span>
        ) : (
          <span className="astral-tag ok">in scope</span>
        )}
      </div>
...
function StepResolve() {
  ...
  const garage = [...S.openings]
    .filter((o) => o.kind === "Garage")
    .sort((a, b) => b.width - a.width)[0];
  const lintelMax = Math.max(
    810,
    ...S.openings.filter((o) => o.kind !== "Garage").map((o) => o.width),
    0,
  );
  return (
    <>
      ...
      <ResCard
        label="Garage door head"
        r={garage ? lintelRule(garage.width) : { v: "—", sed: false, n: "no garage door" }}
      />
    ...
    <>
      ...
      <table className="astral-bom">
        <tbody>
          <tr>
            <td>
              {S.type} · {S.L / 1000}×{S.W / 1000}m · {S.studH / 1000}m stud
            </td>
            <td>{q.floorA} m²</td>
          </tr>
          <tr>
            <td>
              {S.roof} roof {S.pitch}° · {S.cover} · {S.roofCol}
            </td>
            <td>{q.roofA} m²</td>
          </tr>
          <tr>
            <td>
              {S.clad} ({S.cladCol}) · {S.lining} · {S.floor}
            </td>
            <td>{S.insul}</td>
          </tr>
          <tr>
            <td>Joinery: {S.joinery}</td>
            <td>{S.openings.length} openings</td>
          </tr>
          <tr>
            <td>Exposure</td>
            <td>{S.wind}</td>
      ...
      <table className="astral-bom">
        ...
        <tbody>
          <tr>
            <td>Concrete (slab + thickenings)</td>
            <td>{q.concrete} m³</td>
          </tr>
          <tr>
            <td>Mesh SE82</td>
      ...
      {[
        "Engineered foundation + PS1",
        "Roof structure design + PS1 (fabricator)",
        "Garage door head / portal (engineer)",
        "Bracing design (engineer / P21)",
        "Cyclone fixing schedule (engineer)",
        "Cladding system spec (supplier)",
        "Consent pathway + producer statements",
      ].map((t) => (
        <div key={t} className="astral-check">
          <span className="bx" />
          <span>{t}</span>
        </div>
      ))}
    ...
    <div className="astral-panel">
      <div className="astral-rail">
        {STEPS.flatMap((nm, i) => {
          ...
          if (i === 6)
            items.push(
              <div key={`gap-${i}`} className="sctgap" style={{ flexBasis: "100%", height: 0 }} />,
            );
          items.push(
