...
const wallLen = (S: AstralState, w: WallKey) => (w === "N" || w === "S" ? S.L : S.W);
...
export function takeoffStuds(S: AstralState): StudLine[] {
  ...
  return allWalls.map((w) => {
    ...
    const displaced = ops.reduce((a, o) => a + Math.max(0, Math.ceil(o.width / S.spacing) - 1), 0);
...
export function studTotals(lines: StudLine[]): StudTotals {
  const studs_pcs = lines.reduce((a, l) => a + l.common + l.jack + l.trimmer + l.corner, 0);
...
export function takeoffFixings(S: AstralState): FixingRow[] {
  ...
  const studs = takeoffStuds(S).reduce((a, l) => a + l.common + l.jack + l.trimmer + l.corner, 0);
  const hi = ["High", "Very High", "Extra High", "Cyclonic (SED)"].includes(S.wind);
  const st = S.storeys;
  const rows: FixingRow[] = [
    {
      item: "M12 chem anchors (slab → bottom plate)",
      qty: Math.ceil(perim_m / 0.9) * st,
      unit: "ea",
      note: "@900 c/c",
    },
    {
      item: "Cyclone straps (truss tie-down, both ends)",
      qty: trusses * 2,
      unit: "ea",
      note: hi ? "high-wind" : "standard",
    },
    {
      item: "Wire dogs / skew nails (stud → plate)",
      qty: studs * 2,
      unit: "ea",
      note: "top + bottom",
    },
    {
      item: "Lintel hangers / brackets",
      qty: S.openings.length * 2 * st,
      unit: "ea",
      note: "each end",
    },
    {
      item: "Galv. flat-head nails (3.15×75)",
      qty: Math.ceil(perim_m * 30) * st,
      unit: "ea",
      note: "plate fixing",
    },
    {
      item: "Type 17 batten screws (cladding)",
      qty: Math.ceil(((2 * (S.L + S.W) * S.studH * st) / 1e6) * 12),
      unit: "ea",
      note: "≈12/m²",
    },
  ];
...
export function takeoffAreas(S: AstralState): AreaRow[] {
  ...
  const rows: AreaRow[] = [
    {
      item: "Cladding",
      gross_m2: round2(wallA),
      openings_m2: round2(openA),
      net_m2: round2(netExt),
      waste_pct: 10,
      order_m2: round2(netExt * WASTE),
      spec: S.clad,
    },
    {
      item: "Internal lining",
      gross_m2: round2(liningGross),
      openings_m2: 0,
      net_m2: round2(liningGross),
      waste_pct: 10,
      order_m2: round2(liningGross * WASTE),
      spec: S.lining,
    },
    {
      item: "Roofing",
      gross_m2: round2(roofA),
      openings_m2: 0,
      net_m2: round2(roofA),
      waste_pct: 5,
      order_m2: round2(roofA * 1.05),
      spec: `${S.cover} (${S.roofCol})`,
    },
    {
      item: "Building wrap",
      gross_m2: round2(wallA),
      ...
      net_m2: round2(wallA),
      waste_pct: 15,
      order_m2: round2(wallA * 1.15),
      spec: "Self-supporting wrap",
    },
    {
      item: "Roof underlay",
      gross_m2: round2(roofA),
      ...
      net_m2: round2(roofA),
      waste_pct: 15,
      order_m2: round2(roofA * 1.15),
      spec: "Synthetic underlay",
    },
  ];
  ...
  if (st === 2) {
    ...
    rows.push({
      item: "Intermediate floor joists",
      gross_m2: round2(floorA),
      openings_m2: 0,
      net_m2: round2(floorA),
      waste_pct: 10,
      order_m2: round2(floorA * WASTE),
      spec: `≈${round2(joistLm)} lm @450 c/c`,
    });
  }
