...
function R(x: number, y: number, w: number, h: number, fill: string | undefined) {
  return (
    <rect x={x} y={y} width={w} height={h} fill={fill || "none"} stroke={INK} strokeWidth={0.9} />
  );
}
function T(
  x: number,
  ...
  t: string,
  sz = 8.5,
  anc: "start" | "middle" | "end" = "start",
  col = "#3c4a47",
) {
  return (
    <text x={x} y={y} fill={col} fontSize={sz} fontFamily="IBM Plex Mono" textAnchor={anc}>
      {t}
    </text>
  );
}
      ...
      <svg viewBox="-14 -14 628 540" preserveAspectRatio="xMidYMid meet">
        ...
        {ty === "none" &&
          T(
            300,
            220,
            'Select a wall, roof, slab or opening on a plan, then tap "Construction detail".',
            9,
            "middle",
            "#8a7c5e",
          )}
...
function WallDetail({
  ...
  const studs =
    (node?.derived?.studs as { size?: string } | undefined)?.size || studRule(S as never).v;
  ...
  items.push(L1(cx + ww / 2, topY, cx + ww / 2, fy - 13, 1.1));
  for (let y = topY + 14; y < fy - 18; y += 15)
    items.push(L1(cx - ww / 2 + 3, y, cx + ww / 2 - 3, y - 9, 0.4, "#cdbf9f"));
  items.push(L1(cx - ww / 2 - 5, topY - 20, cx - ww / 2 - 5, fy, 0.7, "#a89a78"));
  if (cavity) {
    for (let y = topY - 8; y < fy - 12; y += 24)
      items.push(R(cx - ww / 2 - 10, y, 4, 15, undefined));
    ...
    <g>
      ...
      {cavity && (
        <Lead
          ...
          tx={cx - 150}
          ty={fy - 115}
          t="20mm cavity batten"
        />
      )}
      <Lead
        k="cl"
        x={cx - ww / 2 - (cavity ? 14 : 9)}
        y={fy - 78}
        tx={cx - 150}
        ty={fy - 78}
        t={S.clad}
      ...
      <Lead
        ...
        tx={cx - 150}
        ty={fy - 6}
        t={`plate fix: ${tie.indexOf("chem") > -1 ? "M12 chem anchor @600" : "proprietary anchor @900"}`}
      />
...
function RoofDetail({ S, node }: { S: Pick<DS, "pitch" | "eave" | "cover">; node: Node }) {
  ...
  return (
    <g>
      ...
      <path
        d={`M ${ex - oh - 6} ${ey + oh * slope + 30} a 6 6 0 1 0 12 0`}
        fill="none"
        stroke={INK}
        strokeWidth={0.9}
      />
      ...
      <Lead
        ...
        tx={ex - 150}
        ty={ey - 12}
        t="fix: cyclone clip / strap each truss"
      />
...
function SlabDetail() {
  ...
  for (let x = gx - 156; x < gx + 160; x += 12)
    hatch.push(L1(x, gl, x - 6, gl + 8, 0.4, "#8a7c5e"));
  const mesh: React.ReactNode[] = [];
  for (let x = gx - 92; x < gx + 140; x += 22)
    mesh.push(<circle key={x} cx={x} cy={gl - 38} r={1.3} fill={INK} />);
  return (
    <g>
      ...
      <path
        d={`M ${gx - 150} ${gl - 42} L ${gx + 150} ${gl - 42} L ${gx + 150} ${gl - 30} L ${gx - 104} ${gl - 30} L ${gx - 126} ${gl - 4} L ${gx - 150} ${gl - 4} Z`}
        fill="#efeadd"
        stroke={INK}
        strokeWidth={1.1}
      />
...
function OpeningDetail({ S, node }: { S: Pick<DS, "openings" | "studH">; node: Node }) {
  ...
  const headMM = o
    ? (o.head ??
      (o.kind === "Garage" ? Math.min(S.studH - 200, 2400) : o.kind === "Door" ? 1980 : 2000))
    : 2000;
  ...
  const cx = 300,
    ...
    plY = top - 30;
  const lintel =
    (node?.derived as { lintel?: { v: string } } | undefined)?.lintel?.v || lintelRule(wmm).v;
  const studHatch: React.ReactNode[] = [];
  for (let x = cx - half + 12; x < cx + half; x += 34)
    studHatch.push(L1(x, plY + 14, x, top, 0.6, "#8a7c5e"));
  return (
    <g>
      ...
      {o &&
        o.kind === "Window" &&
        L1(cx - half - 12, top - 2, cx + half + 12, top - 2, 1.3, "#2f6f7e")}
      ...
      {wmm > 3000 &&
        T(cx, plY - 10, "WIDE OPENING — ENGINEERED HEAD (SED)", 8.5, "middle", "#b4472d")}
    </g>
  );
}
