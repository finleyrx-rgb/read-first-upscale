...
import {
  listRenders, uploadRender, deleteRender, readFileAsDataUrl,
  type RenderRow,
...
function stateSnapshot(s: AstralState) {
  return {
    type: s.type, L: s.L, W: s.W, studH: s.studH, spacing: s.spacing,
    roof: s.roof, pitch: s.pitch, openings: s.openings, parts: s.parts,
  };
}
...
export function VisionPanel({ projectId }: { projectId: string | null }) {
  ...
  function applyPicked() {
    ...
    setSketchResult({ ...sketchResult, text: sketchResult.text + `\n\n✓ Applied ${applied}/${sketchResult.actions.length} actions.` });
    setPickedActions(new Set());
  }
  ...
  useEffect(() => {
    if (!projectId || !user) { setSaved([]); return; }
    listRenders(projectId).then(setSaved).catch(() => {});
  }, [projectId, user]);
  ...
  async function saveRender(kind: RenderRow["kind"], base64OrDataUrl: { base64?: string; dataUrl?: string }, caption?: string) {
    if (!projectId) { alert("Save the project first, then save renders."); return; }
    setSaving(true);
    try {
  ...
  async function removeRender(r: RenderRow) {
    ...
    try { await deleteRender(r); setSaved((s) => s.filter((x) => x.id !== r.id)); }
    catch (e) { alert((e as Error).message); }
  }

  return (
    <section style={{
      marginTop: 12, border: "1px solid var(--astral-line, #ddd)", background: "#fff",
      borderRadius: 6, padding: 12,
    }}>
      <header style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
        <strong style={{ fontFamily: "var(--astral-mono)", fontSize: 12 }}>Vision</strong>
        <span style={{ opacity: 0.6, fontSize: 11 }}>sketch → model · site photo → concept render</span>
        <span style={{ flex: 1 }} />
        <button style={{ ...BTN, fontWeight: tab === "sketch" ? 700 : 400 }} onClick={() => setTab("sketch")}>
          ✎ Sketch import
        </button>
        <button style={{ ...BTN, fontWeight: tab === "render" ? 700 : 400 }} onClick={() => setTab("render")}>
          🌄 Site render
        </button>
      </header>
      ...
      {tab === "sketch" && (
        <div style={{ display: "grid", gap: 8 }}>
          ...
          {sketchUrl && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              <div>
                ...
                <img src={sketchUrl} alt="sketch" style={{ width: "100%", maxHeight: 280, objectFit: "contain", border: "1px solid #eee", borderRadius: 4 }} />
              </div>
              <div>
                <div style={{ fontSize: 11, opacity: 0.6, marginBottom: 4 }}>Proposed patches</div>
                {!sketchResult && <div style={{ fontSize: 12, opacity: 0.6 }}>Click <em>Transcribe →</em> to extract actions. You review every one before anything changes.</div>}
                {sketchResult?.error && <div style={{ color: "var(--astral-err, #b14)" }}>⚠ {sketchResult.error}</div>}
                {sketchResult && sketchResult.actions.length === 0 && !sketchResult.error && (
                  <div style={{ fontSize: 12, opacity: 0.7 }}>No patches proposed. {sketchResult.text}</div>
                )}
                {sketchResult && sketchResult.actions.length > 0 && (
                  <>
                    <ul style={{ listStyle: "none", padding: 0, margin: 0, fontSize: 11, fontFamily: "var(--astral-mono)" }}>
                      {sketchResult.actions.map((a, i) => (
                        <li key={i} style={{ display: "flex", gap: 6, alignItems: "flex-start", padding: "3px 0", borderBottom: "1px solid #f0f0f0" }}>
                          <input
                            type="checkbox"
                    ...
                    <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
                      <button style={BTN} onClick={() => setPickedActions(new Set(sketchResult.actions.map((_, i) => i)))}>All</button>
                      <button style={BTN} onClick={() => setPickedActions(new Set())}>None</button>
                      <span style={{ flex: 1 }} />
                      <button style={{ ...BTN, fontWeight: 700 }} disabled={pickedActions.size === 0} onClick={applyPicked}>
                    ...
                    {sketchResult.text && (
                      <details style={{ marginTop: 6 }}>
                        ...
                        <div style={{ fontSize: 11, whiteSpace: "pre-wrap", opacity: 0.8 }}>{sketchResult.text}</div>
                      </details>
                    )}
                  </>
                )}
          ...
          {projectId && sketchUrl && (
            <button style={{ ...BTN, alignSelf: "flex-start" }} disabled={saving}
      ...
      {tab === "render" && (
        <div style={{ display: "grid", gap: 8 }}>
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <span style={{ flex: 1 }} />
            <span style={{ fontSize: 11, opacity: 0.6 }}>Description auto-derived from the live model</span>
          </div>

          <details>
            <summary style={{ fontSize: 11, cursor: "pointer" }}>Building description sent to the renderer</summary>
            <div style={{ fontSize: 11, padding: "6px 8px", background: "#fafaf7", border: "1px solid #eee", borderRadius: 4, fontFamily: "var(--astral-mono)" }}>
              {description}
            </div>
          </details>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {sitePhotoUrl && (
                <div>
                  ...
                  <img src={sitePhotoUrl} alt="site" style={{ width: "100%", maxHeight: 320, objectFit: "contain", border: "1px solid #eee", borderRadius: 4 }} />
                </div>
              )}
              <div>
                <div style={{ fontSize: 11, opacity: 0.6, marginBottom: 4 }}>Concept render</div>
                {renderOut?.error && <div style={{ color: "var(--astral-err, #b14)" }}>⚠ {renderOut.error}</div>}
                {renderOut?.imageBase64 && (
                  <div style={{ position: "relative" }}>
                    <img
                      src={`data:image/png;base64,${renderOut.imageBase64}`}
                      alt="concept render"
                      style={{ width: "100%", maxHeight: 320, objectFit: "contain", border: "1px solid #eee", borderRadius: 4 }}
                    />
                ...
                {!renderOut && !renderBusy && <div style={{ fontSize: 12, opacity: 0.6 }}>Awaiting render.</div>}
                {renderOut?.imageBase64 && projectId && (
                  <button style={{ ...BTN, marginTop: 6 }} disabled={saving}
                    onClick={() => saveRender("site-render", { base64: renderOut.imageBase64 }, "Site concept render")}>
                    💾 Save to project
                  </button>
                )}
              </div>
      ...
      {saved.length > 0 && (
        <div style={{ marginTop: 10, borderTop: "1px solid #eee", paddingTop: 8 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: 6 }}>
            {saved.map((r) => (
              <div key={r.id} style={{ border: "1px solid #eee", borderRadius: 4, padding: 4, fontSize: 10 }}>
                {r.url ? (
                  <a href={r.url} target="_blank" rel="noreferrer">
                    <img src={r.url} alt={r.caption ?? r.kind} style={{ width: "100%", height: 80, objectFit: "cover", borderRadius: 3 }} />
                  </a>
                ) : <div style={{ height: 80, background: "#f4f4f4" }} />}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 3 }}>
                  <span style={{ ...PILL, background: r.kind === "site-render" ? "#2c5d36" : "#5d3c2c" }}>{r.kind}</span>
                  <button onClick={() => removeRender(r)} style={{ ...BTN, padding: "1px 5px", fontSize: 10 }}>✕</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {!user && (
        <div style={{ marginTop: 8, fontSize: 11, opacity: 0.6 }}>
          Sign in to save sketches and renders to a project. The transcription and render features work without sign-in.
        </div>
      )}
    </section>
  );
