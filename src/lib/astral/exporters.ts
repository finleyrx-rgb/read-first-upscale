...
function drawWatermark(pdf: jsPDF, pageW: number, pageH: number, elaborated = false) {
  ...
  const text = elaborated
    ? "ELABORATED FROM SOURCE PLAN — REVIEW AGAINST ORIGINAL — NOT FOR CONSTRUCTION"
    : "CONCEPT — INDICATIVE, NOT FOR CONSTRUCTION";
  pdf.setFontSize(elaborated ? 44 : 64);
...
function drawChrome(pdf: jsPDF, pageW: number, pageH: number, S: AstralState, info: ChromeInfo) {
  ...
  drawWatermark(pdf, pageW, pageH, !!S.sourceElaborated);
}
...
function renderCover(
  ...
  // Disclaimer
  const elaborated = !!useAstral.getState().sourceElaborated;
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(9);
  pdf.setTextColor(ACCENT);
  pdf.text(
  ...
  pdf.text(
    elaborated
      ? "This set was elaborated by Project Astral from an uploaded source plan. Every drawing is indicative and must be reviewed against the original author's documents and confirmed by a licensed design professional before construction. SED items require PS1."
