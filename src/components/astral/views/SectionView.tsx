...
export function SectionView() {
  ...
  if (S.roof === "Flat") {
    ...
    rpts = [
      [x0 - eaveOff, eave - apexP * 0.05],
      [x0 + w + eaveOff, eave - apexP * 0.05],
      [x0 + w + eaveOff, eave],
      [x0 - eaveOff, eave],
    ];
  ...
  const cutThroughOps = S.openings.filter((o) => {
    ...
    return (
      Math.abs(o.off - (perpmm - cutAtMM)) < o.width / 2 || Math.abs(o.off - cutAtMM) < o.width / 2
    );
  });
  ...
  for (let xx = x0 - 30; xx < x0 + w + 30; xx += 8) {
    earthHatch.push(
      <line
        key={xx}
        x1={xx}
        y1={base + (slabT + footing) * sc}
        x2={xx + 5}
        y2={base + (slabT + footing) * sc + 5}
        stroke="#7a6f57"
        strokeWidth={0.4}
      />,
    );
    ...
    <ClickLayer>
      <svg viewBox="-14 -14 628 540" preserveAspectRatio="xMidYMid meet">
        ...
        <rect
          ...
          fill={INK}
          fillOpacity={0.82}
          data-node-id={`wall-${cut === "cross" ? "W" : "S"}`}
          data-node-type="Wall"
        />
        <rect
          x={x0 + w - wallT}
          ...
          fill={INK}
          fillOpacity={0.82}
          data-node-id={`wall-${cut === "cross" ? "E" : "N"}`}
          data-node-type="Wall"
        />
        ...
        {firstFloorY !== null && (
          <g pointerEvents="none">
            ...
            <line
              ...
              strokeDasharray="4 2"
            />
            <text x={x0 + w + 10} y={firstFloorY + 2} fill="#7a4a16" fontSize={7} fontFamily={MONO}>
              1st floor
            </text>
          </g>
        )}
        ...
        <g transform={`translate(${x0 - 60} ${eave - 70})`} pointerEvents="none">
          <rect
            ...
            opacity={0.96}
          />
          <text x={0} y={-2} fill="#3c4a47" fontSize={7.5} fontFamily={MONO} letterSpacing="0.1em">
            INSULATION · {S.insul.toUpperCase()}
          </text>
          ...
          {[
            ...
            <g key={lab} transform={`translate(0 ${12 + i * 12})`}>
              <text x={0} y={0} fill={INK} fontSize={8.5} fontFamily={MONO}>
                {lab}
              </text>
              <text
                x={126}
                y={0}
                fill={Rv.wall === "—" && i === 0 ? "#b4472d" : INK}
                fontSize={8.5}
                fontFamily={MONO}
                textAnchor="end"
                fontWeight={600}
        ...
        <text
          x={x0 + w / 2}
          y={base + footing * sc + 22}
          fill={DATUM}
          fontSize={11}
          fontFamily={MONO}
          textAnchor="middle"
        >
          SECTION {tag}–{tag} · {cut === "cross" ? "across width" : "along length"} · cut @{" "}
          {cutAtMM}mm · {S.roof} {S.pitch}°
        </text>
        <text
          x={x0 + w / 2}
          y={base + footing * sc + 36}
          fill="#7a6f57"
          fontSize={8.5}
          fontFamily={MONO}
          textAnchor="middle"
      ...
      <ScaleChip
        label={`section ${tag}–${tag} · ${cut === "cross" ? "cross (across width)" : "longitudinal (along length)"} · cut @ ${cutAtMM}mm · indicative`}
      />
    </ClickLayer>
  );
}
