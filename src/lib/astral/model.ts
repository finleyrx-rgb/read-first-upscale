...
export type ModelNode = {
  ...
  compliance: { rule: string; value: string; status: "ok" | "sed"; basis: string; raw: RuleResult }[];
};

export type Model = { root: ModelNode; byId: Record<string, ModelNode> };

const wallLen = (S: AstralState, w: "N" | "S" | "E" | "W") =>
  w === "N" || w === "S" ? S.L : S.W;
...
export function studRule(S: AstralState): RuleResult {
  ...
  if (S.studH >= 3000)
    return eh
      ? { v: "140×45 SG8 " + sp, sed: false, n: `3604 T8.2 · ${S.wind} · LD ${(loadedDim / 1000).toFixed(1)}m · indicative` }
...
export function lintelRule(span: number): RuleResult {
  if (span > 3000) return { v: "Engineered head / portal", sed: true, n: `${span}mm > 3604 lintel range → SED` };
...
export function roofRule(S: AstralState): RuleResult {
  ...
  if (S.struct.includes("truss"))
    return { v: `Prefab ${S.roof === "Hip" ? "hip " : ""}trusses @900`, sed: true, n: `${(span / 1000).toFixed(1)}m span · fabricator PS1` };
  if (span > 6000) return { v: "Engineered rafters/beam", sed: true, n: `${(span / 1000).toFixed(1)}m → SED` };
...
export function tieRule(S: AstralState): RuleResult {
  ...
  return hi
    ? { v: "Cyclone straps + M12 chem anchors @600", sed: S.wind === "Cyclonic (SED)", n: "Strap every truss/rafter, both ends" }
...
function mk(id: string, type: string, name: string, parent: string | null, params: Record<string, unknown> = {}, geometry: Record<string, unknown> = {}): ModelNode {
...
export function buildModel(S: AstralState): Model {
  ...
  S.openings.forEach((o) => {
    const op = add(mk(`opening-${o.id}`, "Opening", o.kind, `wall-${o.wall}`, { kind: o.kind, width: o.width, off: o.off, wall: o.wall }));
    const pw = byId[`wall-${o.wall}`];
    if (pw) pw.children.push(op.id);
  });

  const roof = add(mk("roof", "RoofSystem", `${S.roof} roof`, "building", { form: S.roof, pitch: S.pitch, cover: S.cover, struct: S.struct }, { pitch: S.pitch }));
  bld.children.push(roof.id);

  S.parts.forEach((p) => {
    const pt = add(mk(`part-${p.id}`, "Partition", "Partition", "building", { dir: p.dir, off: p.off, start: p.start, len: p.len, door: p.door, doorW: p.doorW, doorOff: p.doorOff, type: p.type }));
  ...
  Object.values(byId).forEach((n) => {
    if (n.type === "Wall") {
      ...
      n.derived = { studs: { count: Math.round(len / S.spacing) + 1, size: studRule(S).v }, topPlate: 2, bottomPlate: 1 };
      const r = studRule(S);
      n.compliance.push({ rule: "stud", value: r.v, status: r.sed ? "sed" : "ok", basis: r.n, raw: r });
      ...
      n.compliance.push({ rule: "lintel", value: r.v, status: r.sed ? "sed" : "ok", basis: r.n, raw: r });
    } else if (n.type === "RoofSystem") {
      const r = roofRule(S);
      n.derived = { system: S.struct.includes("truss") ? "Prefab trusses" : "Cut rafters", members: Math.round(S.L / 900), purlins: Math.round(((S.L * S.W) / 1e6) / 0.9) };
      n.compliance.push({ rule: "structure", value: r.v, status: r.sed ? "sed" : "ok", basis: r.n, raw: r });
      ...
      [
        ...
        { id: "lintels", r: lintelRule(Math.max(810, ...S.openings.filter((o: Opening) => o.kind !== "Garage").map((o: Opening) => o.width), 0)) },
      ].forEach(({ id, r }) => n.compliance.push({ rule: id, value: r.v, status: r.sed ? "sed" : "ok", basis: r.n, raw: r }));
    }
  });
