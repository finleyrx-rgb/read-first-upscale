...
export function PlanDimensions() {
  ...
  const wOps = opsAt("W");
  const eOps = opsAt("E");
  const partsAcrossX = S.parts
    .filter((p) => p.dir === "Across width")
    .map((p) => p.off)
    .sort((a, b) => a - b);
  const partsAlongY = S.parts
    .filter((p) => p.dir === "Along length")
    .map((p) => p.off)
    .sort((a, b) => a - b);
  ...
  let bRow = y0 + h + 22;
  if (sOps.length) {
    out.push(rowH(dedup([0, ...sOps, S.L]), bRow, "WINDOWS", "s-win"));
    bRow += 18;
  }
  if (partsAcrossX.length) {
    out.push(rowH(dedup([0, ...partsAcrossX, S.L]), bRow, "INTERIOR WALLS", "s-int"));
    bRow += 18;
  }
  ...
  let lCol = x0 - 22;
  if (wOps.length) {
    out.push(rowV(dedup([0, ...wOps, S.W]), lCol, "WINDOWS", "w-win"));
    lCol -= 18;
  }
  if (partsAlongY.length) {
    out.push(rowV(dedup([0, ...partsAlongY, S.W]), lCol, "INTERIOR WALLS", "w-int"));
    lCol -= 18;
  }
  ...
  let rCol = x0 + w + 22;
  if (eOps.length) {
    out.push(rowV(dedup([0, ...eOps, S.W]), rCol, "WINDOWS", "e-win"));
    rCol += 18;
  }
